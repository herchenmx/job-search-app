...
export default function RoleRubricPage() {
  ...
  async function saveRubric(content: string) {
    ...
    try {
      ...
      // Clear all role match scores so cron re-analyses with updated rubric
      await supabase
        .from('jobs')
          ...
          {messages.length > 0 && (
            <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={resetConversation} className="text-sm text-red-500 hover:underline">
                  Start over
                </button>
              </div>
            </div>
      ...
      {mode === 'paste' && (
        <div className="space-y-4">
          ...
          <label className="block text-sm font-medium text-gray-700">
            Paste your role preferences rubric (markdown or table format)
          </label>
          <textarea
            value={pasteValue}
